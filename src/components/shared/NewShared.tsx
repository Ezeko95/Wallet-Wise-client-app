...
const NewShared: React.FC = () => {
  ...
  const handleSelf = async () => {
    ...
    setArrayRender([
      ...arrayRender,
      { name: 'Self Expense', expense: parseFloat(selfExpense) },
    ]);
    setSelfExpense('');
  };
  ...
  const handlerSubmit = async () => {
    ...
    const response = await axios.post(
      `${base_URL}/shared/${ide[ide.length - 1]}`,
      info,
    );
    setSelfExpense('');
    setRoom('');
    ...
    <View style={styles.container}>
      ...
      <View style={styles.inputContainer}>
        ...
        <View style={styles.containerButtons}>
          <TouchableOpacity
            onPress={() => handleParticipant()}
            disabled={!name || !expense}>
      ...
      <ScrollView>
        <View style={styles.containerAllDetails}>
          ...
          {arrayRender.length > 0 &&
            arrayRender.map((participant, index) => {
              return (
